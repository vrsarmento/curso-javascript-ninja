(function(){
	...
	*/
	...
	para exemplificar.
	*/
	var markup = '<main>\n  <div class="container">\n    <span class="text date button_primary btn"></span>\n    <p class=\'excerpt\'></p>\n  </div>\n</main>';
	...
	function hasClass( markup, cssClass ){
		...
		var arrayClasses = [];
		markup.replace( regex, function(all, classes){
			arrayClasses.push(classes);
		});
		
		arrayClasses = arrayClasses.map(function(current){
			return current.split(' ');
		}).reduce(function(all, item){
			return all.concat( item );
		});

		return arrayClasses.indexOf( cssClass ) !== -1 ? true : false;
	}

	var tests = [ 'container', 'text', 'date', 'excerpt', 'main', 'button_primary', '_btn', 'btn_secondary'];
	...
	function hasClass2( markup, cssClass ){
		...
		// outra opção, menos específica, mas que diminui performance
		return regex.test( markup );
	}

	tests.forEach(function(item){
		console.log( hasClass2( markup, item ) + ' para a classe ' + item );
	});
